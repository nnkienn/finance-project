...
public class TransactionService implements RecurringPostingPort {
	...
	public TransactionResponse createTransaction(TransactionRequest request) {
		...
		if (request.getType() != TransactionType.SAVING && request.getUserCategoryId() != null) {
			userCategory = userCategoryRepository.findByIdAndUserId(request.getUserCategoryId(), user.getId())
		...
		try {
			var dto = TransactionEventDTO.builder().transactionId(tx.getId()).userId(tx.getUser().getId())
					.type(tx.getType()) // enum OK
					.method(tx.getPaymentMethod()) // enum OK
					.amount(tx.getAmount()).transactionDate(tx.getTransactionDate()).note(tx.getNote())
					.userCategoryId(tx.getUserCategory() != null ? tx.getUserCategory().getId() : null)
					.savingGoalId(tx.getSavingGoal() != null ? tx.getSavingGoal().getId() : null).build();

			outboxService.saveEvent("Transaction", String.valueOf(tx.getId()), "transaction.created", dto);
	...
	public List<TransactionResponse> getUserTransactions() {
		User user = SecurityUtils.getCurrentUser();
		return transactionRepository.findByUserId(user.getId()).stream().map(TransactionMapper::toResponse).toList();
	}
	...
	public TransactionResponse updateTransaction(Long id, TransactionRequest request) {
		...
		tx.setNote(request.getNote());
		tx.setTransactionDate(
				request.getTransactionDate() != null ? request.getTransactionDate() : tx.getTransactionDate());
	...
	public void deleteTransaction(Long id) {
		...
		if (tx.getType() == TransactionType.SAVING && tx.getSavingGoal() != null) {
			savingGoalService.adjustAmountForUpdate(tx.getSavingGoal(), tx.getAmount().negate(),
					"Revert due to transaction delete");
		}
	...
	@Transactional(readOnly = true)
	public List<TransactionResponse> getTransactionsFiltered(LocalDateTime startDate, LocalDateTime endDate,
			TransactionType type, Long categoryId, PaymentMethod paymentMethod) {
		User user = SecurityUtils.getCurrentUser();
		...
		return transactionRepository
				.findAll(TransactionSpecification.withFilters(user, startDate, endDate, type, category, paymentMethod))
				.stream().map(TransactionMapper::toResponse).toList();
	}

	@Transactional(readOnly = true)
	public Page<TransactionResponse> getTransactionsFilteredPaged(LocalDateTime startDate, LocalDateTime endDate,
			TransactionType type, Long categoryId, PaymentMethod paymentMethod, Pageable pageable) {
		User user = SecurityUtils.getCurrentUser();
		UserCategory category = null;
	...
	public List<TransactionResponse> getLatestTransactions(int limit) {
		User user = SecurityUtils.getCurrentUser();
		return transactionRepository.findTop10ByUserOrderByTransactionDateDesc(user).stream().limit(limit)
				.map(TransactionMapper::toResponse).toList();
	}

	@Transactional(readOnly = true)
	public TimeseriesResponse getTimeseries(LocalDateTime start, LocalDateTime end, String granularity, String scope) {
		User user = SecurityUtils.getCurrentUser();
